...
public class MockServiceTest {
    ...
    @Test
    public void 어떠한챔피언이름을입력해도베인리턴테스트() {
        Champion champion = mock(Champion.class);
    ...
    @Test
    public void 어떠한포지션을입력해도미드리턴테스트() {
        Champion champion = mock(Champion.class);
    ...
    public void shouldOneTimeRunForChampionArraySize() {
        ...
        System.out.println("size :: " + mockChampions.size()); //0 출력됨 가짜객체이기 때문
        verify(mockChampions, times(1)).size(); //size 한 번 검사했는지 테스트
    }
    ...
    public void shouldAtLeastTwoTimesForChampionGetName() {
        ...
        System.out.println("name :: " + champion.getName());
        verify(champion, atLeast(2)).getName(); //getName 두 번 이상 검사했는지 테스트
    }

    // 4-2. champion 객체에서 이름을 가져오는 로직이 최대 3번 이하 실행되면 Pass 하는 로직을 작성하세요.
    @Test
    public void shouldAtMostThreeTimeForChampionGetName() {
        Champion champion = mock(Champion.class);
        champion.setName("조이");
        champion.setPosition("미드");
        champion.setHasSkinCount(3);
    ...
    public void twoLists() {
        ...
        champions.add(new Champion("티모", "미드", 1));
        champions.add(new Champion("조이", "바텀", 1));
        champions.add(new Champion("애쉬", "미드", 1));
    ...
    @Test
    public void skinNumTest() {
        when(mockService.findByName(anyString())).thenReturn(new Champion("조이", "미드", 3));
        assertThat(mockService.findByName("애쉬").getHasSkinCount(), is(3));
        verify(mockRepository, times(1)).findByName(anyString());
    }
...
//        assertThat(mockService.findByName("조이").getHasSkinCount(), is(1)); when에 AllChampion으로 해놔서 ByName 안 됨.
        assertThat(mockService.findAllChampions().get(1).getName(), is("조이"));
